...
fun describe(result:TransferResult): String {
    return when (result){
        ...
        is TransferResult.Success -> "Transfer Sucessful. Confirmation: ${result.confirmationId}"
...
fun main() = runBlocking {
    ...
    val overdraftAccounts = accounts.filter { account ->
        account.balance < 0
    }
    ...
    overdraftAccounts.forEach { account ->
        println("${account.name}: ${account.balance.asCurrency()}")
    }
}
